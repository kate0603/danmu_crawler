...
from douyin_live.config import LIVE_GIFT_LIST
from douyin_live.src.utils.logger import logger
from utils.live_log import LiveLog
import re
import time
import requests
import websocket
from douyin_live.src.utils.ws_send import ws_send, ws_sender
from douyin_live.src import live_rank
from douyin_live.src.utils.common import GlobalVal
from protobuf_inspector.types import StandardParser
from google.protobuf import json_format
from douyin_live.proto.dy_pb2 import PushFrame
from douyin_live.proto.dy_pb2 import Response
from douyin_live.proto.dy_pb2 import MatchAgainstScoreMessage
from douyin_live.proto.dy_pb2 import LikeMessage
from douyin_live.proto.dy_pb2 import MemberMessage
from douyin_live.proto.dy_pb2 import GiftMessage
from douyin_live.proto.dy_pb2 import ChatMessage
from douyin_live.proto.dy_pb2 import SocialMessage
from douyin_live.proto.dy_pb2 import RoomUserSeqMessage
from douyin_live.proto.dy_pb2 import UpdateFanTicketMessage
from douyin_live.proto.dy_pb2 import CommonTextMessage
from douyin_live.proto.dy_pb2 import ProductChangeMessage

import threading


class WebSocketClient(threading.Thread):
    def __init__(self, live_room_title, live_room_id, ttwid, game_id):
        threading.Thread.__init__(self)
        self.start_time = time.time()
        self.ttwid = ttwid
        self.live_room_id = live_room_id
        self.live_room_title = live_room_title
        self.game_id = game_id

        websocket.enableTrace(False)
        # 拼接获取弹幕消息的websocket的链接
        webSocketUrl = (
            "wss://webcast3-ws-web-lq.douyin.com/webcast/im/push/v2/?app_name=douyin_web&version_code=180800&webcast_sdk_version=1.3.0&update_version_code=1.3.0&compress=gzip&internal_ext=internal_src:dim|wss_push_room_id:"
            + self.live_room_id
            + "|wss_push_did:7188358506633528844|dim_log_id:20230521093022204E5B327EF20D5CDFC6|fetch_time:1684632622323|seq:1|wss_info:0-1684632622323-0-0|wrds_kvs:WebcastRoomRankMessage-1684632106402346965_WebcastRoomStatsMessage-1684632616357153318&cursor=t-1684632622323_r-1_d-1_u-1_h-1&host=https://live.douyin.com&aid=6383&live_id=1&did_rule=3&debug=false&maxCacheMessageNumber=20&endpoint=live_pc&support_wrds=1&im_path=/webcast/im/fetch/&user_unique_id=7188358506633528844&device_platform=web&cookie_enabled=true&screen_width=1440&screen_height=900&browser_language=zh&browser_platform=MacIntel&browser_name=Mozilla&browser_version=5.0%20(Macintosh;%20Intel%20Mac%20OS%20X%2010_15_7)%20AppleWebKit/537.36%20(KHTML,%20like%20Gecko)%20Chrome/113.0.0.0%20Safari/537.36&browser_online=true&tz_name=Asia/Shanghai&identity=audience&room_id="
            + self.live_room_id
    ...
    def unPackWebcastCommonTextMessage(self, data):
        ...
        logger.info(
            "[unPackWebcastCommonTextMessage] [] [房间Id："
            + self.live_room_id
            + "] | "
            + log
        )
    ...
    def WebcastProductChangeMessage(self, data):
        ...
        log = json.dumps(data, ensure_ascii=False)
        logger.info(
            "[WebcastProductChangeMessage] [] [房间Id："
            + self.live_room_id
            + "] | "
            + log
        )
    ...
    def unPackWebcastUpdateFanTicketMessage(self, data):
        ...
        log = json.dumps(data, ensure_ascii=False)
        logger.info(
            "[unPackWebcastUpdateFanTicketMessage] [] [房间Id："
            + self.live_room_id
            + "] | "
            + log
        )
    ...
    def unPackWebcastRoomUserSeqMessage(self, data):
        ...
        log = json.dumps(data, ensure_ascii=False)
        logger.info(
            "[unPackWebcastRoomUserSeqMessage] [] [房间Id："
            + self.live_room_id
            + "] | "
            + log
        )
    ...
    def unPackWebcastChatMessage(self, data):
        ...
        event_time = datetime.fromtimestamp(int(data["eventTime"]))
    ...
    def unPackWebcastGiftMessage(self, data):
        ...
        try:
            ...
            # 对特殊礼物单独统计
            if (
                gift_name in LIVE_GIFT_LIST
                and gift_traceId not in GlobalVal.gift_id_list
            ):
    ...
    def onError(self, ws, error):
        logger.error(
            "[onError] [webSocket Error事件] [房间Id：" + self.live_room_id + "]"
        )
    ...
    def onClose(self, ws, a, b):
        ...
        logger.info(
            "[onClose] [webSocket Close事件] [房间Id：" + self.live_room_id + "]"
        )
        # 直播结束退出程序
    ...
    def onOpen(self, ws):
        ...
        logger.info("[onOpen] [webSocket Open事件] [房间Id：" + self.live_room_id + "]")
